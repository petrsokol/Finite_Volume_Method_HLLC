...
Conservative Bound::updateInletCell(const Conservative &innerW) {
    // compute primitive variables for the inner cell's vector W
    Primitive innerPV = Primitive::computePV(innerW);

    Bound::p_infty = innerPV.p;
    double p_0 = Def::p_inlet;
    double mach_infty = 5;
    if (Def::mach_infty < 1) {
         mach_infty = sqrt(2 / (Def::KAPPA - 1) * (pow(
                 p_0 / Bound::p_infty,(Def::KAPPA - 1) / Def::KAPPA)-1));
    } else {
         mach_infty = Def::mach_infty;
    }

    Bound::rho_infty = Def::rho_inlet *
            pow(Bound::p_infty / p_0, 1 / Def::KAPPA);
    ...
    double rhoE = Bound::p_infty /
            (Def::KAPPA - 1) + 0.5 * Bound::rho_infty * pow(U, 2);
...
Conservative Bound::updateOutletCell(const Conservative &innerW) {
    ...
    if (Def::mach_infty < 1) {
        double p_2 = Def::p_outlet;
        double rhoE = p_2 / (Def::KAPPA - 1) + 0.5 *
                innerPV.rho * pow(innerPV.U, 2);
...
Conservative Bound::updateWallCell(const Conservative &innerW,
    ...
    Conservative outerW;
    outerW.r1 = innerW.r1;
    outerW.r2 = innerW.r1 * (uInner - 2 *
            (uInner * nx + vInner * ny) * nx);
    outerW.r3 = innerW.r1 * (vInner - 2 *
            (uInner * nx + vInner * ny) * ny);
    outerW.r4 = innerW.r4;
